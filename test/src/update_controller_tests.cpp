...
TEST(UpdateControllerTests, update_announce_interval) {
  ...
  // Change back to default: 1800
  response = http::get("/"+ config::get_instance()->site_password +"/update?action=update_announce_interval&new_announce_interval=1800");

  EXPECT_TRUE( response.find("Edited announce interval to 1800") != std::string::npos );
}
